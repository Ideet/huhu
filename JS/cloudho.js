...
~function() {
    ...
    (function initPage() {
        ...
        if (!resultContainer) {
            ...
            resultContainer.textContent = '正在初始化桥接和调用API...';
        }
        ...
        setTimeout(() => {
            ...
            apiCalls.forEach(({ api, params, label }) => {
                ...
                wiseopercampaign[module][method](
                    params,
                    (data) => {
                        resultContainer.innerHTML += `<div style="color:#0f0;margin:10px 0">✅ ${label} succeed：${JSON.stringify(data)}</div>`;
                    },
                    (err) => {
                        resultContainer.innerHTML += `<div style="color:#f00;margin:10px 0">❌ ${label} error：${JSON.stringify(err)}</div>`;
                    }
                );
            });
            ...
            setTimeout(() => {
                ...
                apiSyncCalls.forEach(({ api, params, label }) => {
                    ...
                    if(window.wiseopercampaignbridge){
                        ...
                        resultContainer.innerHTML += `<div style="color:#ff0;margin:10px 0">📱 ${label} result：${JSON.stringify(result)}</div>`;
                    }
                });
            }, 100);
        }, 100);
