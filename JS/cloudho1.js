...
~function() {
    ...
    // 等待桥接对象加载
    let checkCount = 0;
    const maxChecks = 30; // 最多检查30次，每次100ms，共3秒
    
    function checkBridgeAndInit() {
        checkCount++;
        
        // 检查必要的桥接对象
        if (typeof window.wiseopercampaignbridge === 'undefined') {
            showResult(`⏳ 等待 wiseopercampaignbridge 加载... (${checkCount}/${maxChecks})`);
            
            if (checkCount >= maxChecks) {
                showResult('❌ 超时: wiseopercampaignbridge 未加载，可能当前环境不支持', true);
                return;
            }
            
            setTimeout(checkBridgeAndInit, 100);
            return;
        }
        
        showResult('✅ wiseopercampaignbridge 已加载，开始初始化...');
    ...
    // 启动检查
    setTimeout(checkBridgeAndInit, 0);
